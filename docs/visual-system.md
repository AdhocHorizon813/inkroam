...
2026-09-26 经用户同意，浅色图片背景（暮色都市、夕空町市、自定义图片）的画布混色从18%/11%降至12%/6%，让主题色更像局部环境光，保留原图色彩。纯色、极光、深色与材质幕层比例不变，遮罩和模糊不变。此为设计取舍，不是实测最优值。

阅读设置的「主题色混合背景」提供开启/关闭，位于氛围色之前。状态为 `backgroundTint`，根属性为 `data-background-tint="on|off"`；默认开启，旧偏好缺值或坏值也回到开启，保存沿用现有localStorage，恢复默认重新开启。与氛围色一样独立于「默认设置」的自动管理，切换明暗不会重置用户选择；纸媒下控件禁用。

开启时 `--background-tint: var(--modern-accent)`，保持原混色比例；关闭时为 `transparent`，只消除背景装饰层的主题色成分。图片、背景模糊、明暗遮罩、按钮/链接的主题色不变。**主题纯色画布是明确选择的纯色，不是叠加染色，所以仍保留主题色。** 极光的主题色光斑受开关控制，但固定青/粉光斑不受影响；关闭不等于把所有背景变成灰色。

#### 实际混色比例

以下来自 `app/assets/css/main.css` 的渐变声明。比例是渐变指定位置处，主题色与透明色混合的权重，**不是整个背景的最终透明度，也不能把两层比例相加**。

| 图层/模式 | 主题色比例 | 位置与衰减 |
| --- | --- | --- |
| 浅色通用画布 `.ambient-backdrop` | 18%、11% | 圆心分别在16% 10%、84% 72%；向外至38%、42%渐隐为透明 |
| 浅色图片画布（art / dusk / custom 覆盖通用值） | 12%、6% | 圆心与衰减范围不变，仅降低混色权重 |
| 深色静谧纯色画布 | 8%、5% | 圆心18% 12%、82% 76%；至34%、38%渐隐 |
| 浅色静谧纯色画布（最终覆盖规则） | 9%、6% | 圆心18% 12%、82% 76%；至36%、40%渐隐 |
| 极光的主题色光斑 `.ambient-aurora` | 48% | 椭圆中心24% 32%，至34%渐隐；还会受极光整层opacity影响 |
| 通用/深色液态玻璃幕 `.ambient-vignette` | 5% | 135°线性渐变，主题色节点在72% |
| 通用/深色亚克力幕 | 8% | 椭圆中心25% 18%，至40%渐隐 |
| 通用/深色云母幕 | 7% | 椭圆中心22% 14%，至42%渐隐 |
| 浅色液态玻璃追加幕 `::before` | 6% | 135°线性渐变，主题色节点在72% |
| 浅色亚克力追加幕 | 9% | 椭圆中心24% 16%，至42%渐隐 |
| 浅色云母追加幕 | 8% | 椭圆中心22% 14%，至42%渐隐 |

样式表较早的通用画布36%光斑已被后续 `.ambient-backdrop` 背景规则覆盖；早期浅色静谧纯色7%/4%也被后面的9%/6%覆盖。它们仍在源码中，不应被当成当前实际比例。浅色追加幕与通用幕存在叠加关系，并非简单替换；幕层opacity还受「背景遮罩透明度」控制。图片opacity、底图颜色、模糊、饱和度与明暗处理都会进一步影响最终像素。

修改混色强度时，搜索 `--background-tint` 对应渐变的百分比，不要直接改 `--modern-accent` 或界面主题变量。用户本轮已肉眼确认开关造成可见差异；尚未进行固定坐标像素采样或所有主题组合的浏览器验收。`check-reading-appearance.mjs` 已覆盖默认开启、旧值兼容、关闭保存、明暗切换不重置、恢复默认及背景混色通道隔离。
